            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if vm.totalRecords > 0 {
                        Text("\(vm.totalRecords.formatted()) acknowledged points")
                            .font(.headline)
                            .foregroundStyle(.primary)
                    }
                    if vm.isAnySyncRunning {
                        Text("\(vm.sessionAcknowledgedPoints.formatted()) acknowledged this run")
                            .font(.headline)
                            .foregroundStyle(.green)
                    }
                    Text("\(vm.pendingQueueCount) pending \(vm.pendingQueueCount == 1 ? "batch" : "batches")")
                        .font(.caption)
                        .foregroundStyle(vm.pendingQueueCount == 0 ? Color.secondary : Color.orange)
                }
                Spacer()
            }
