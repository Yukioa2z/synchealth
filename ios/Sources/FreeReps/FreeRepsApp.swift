...
final class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(
        ...
        Task { @MainActor in
            // Register observers before waiting on the network-backed queue drain so launch-time
            // connectivity problems cannot delay HealthKit wake registration.
            BackgroundSyncManager.shared.startObserving()
            let queueState = SyncState()
            let queueDrainer = SyncService(syncState: queueState)
            await queueDrainer.drainPendingQueue(config: .load())
        }
    ...
    func applicationProtectedDataDidBecomeAvailable(_ application: UIApplication) {
        Task { @MainActor in
            BackgroundSyncManager.shared.retryPendingUpdatesAfterUnlock()
        }
    }

    private func registerBackgroundTasks() {
        BGTaskScheduler.shared.register(
            forTaskWithIdentifier: "com.example.synchealth.sync",
            using: nil
        ) { task in
    ...
    private func handleBackgroundSync(task: BGProcessingTask) {
        ...
        let syncTask: Task<Void, Never> = Task { @MainActor in
            ...
            } else if state.hasCompletedFullSync, state.lastSyncDate != nil {
